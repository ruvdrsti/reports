...
\begin{equation}\label{eq:slater}
    \Psi = 1/\sqrt{N!}\begin{vmatrix}
        \chi_1(\boldsymbol{x}_1) & \cdots & \chi_n(\boldsymbol{x}_1) \\
        \cdots & \cdots & \cdots \\
        \chi_1(\boldsymbol{x}_n) & \cdots & \chi_n(\boldsymbol{x}_n)\\
    \end{vmatrix}
\end{equation}
When we look at the individual functions $\chi_i(x_j)$ we can state Equation 
\ref{eq:spinor}

\begin{equation}\label{eq:spinor}
    \chi_i(x_j) = \psi_i(\boldsymbol{x}_j)\gamma(\boldsymbol{\omega}_j)
\end{equation}
...
\begin{enumerate}
    ...
    very good yet, since off course the core Hamiltonian is not the same as the 
    ...
    Off course the functions do have to be "well-behaved". The size however, will always be the same.
    ...
    So we need to end up with an expansion that is only dependant on the spacial
...
These are constans, so we can safely extract them from the integrals to form
