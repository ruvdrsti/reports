...
class molecule:
    ...
    def getDensityMatrix(self):
        ...
        A = 2*np.einsum("pq, qr->pr", C[:, :self.occupied], C[:, :self.occupied].T)
        return A


    def displayFockMatrix(self):
        """Will display the Fock matrix"""
        C = np.einsum("nopq,pq->no", self.displayElectronRepulsion(), self.getDensityMatrix())
        J = np.einsum("npoq,pq->no", self.displayElectronRepulsion(), self.getDensityMatrix())
        self.fockMatrix = self.displayHamiltonian() + C - 0.5*J
...
def iterator(target_molecule):
    ...
    while not convergence and itercount < 50:
        ...
        print(f"""iteration: {itercount}, E_tot: {E_total: .8f}, E_elek: {E_new: .8f}, 
        deltaE: {E_new - E_old: .8f}, rmsD: {rms_D: .8f}""")
